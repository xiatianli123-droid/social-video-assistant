#!/usr/bin/env python3
"""AIG image generation/editing CLI based on the Codex image generation script.

Uses OPENAI_API_KEY from the Codex auth.json for authentication and reads the
OpenAI-compatible base URL from the Codex config provider `model_providers.Joinin`.
"""
...
DEFAULT_PROVIDER_NAME = "Joinin"
DEFAULT_REQUEST_TIMEOUT_SECONDS = 180.0
CODEX_AUTH_ENV = "CODEX_AUTH_PATH"
CODEX_AUTH_API_KEY_FIELD = "OPENAI_API_KEY"
CODEX_CONFIG_ENV = "CODEX_CONFIG_PATH"
REQUEST_TIMEOUT_ENV = "AIG_REQUEST_TIMEOUT_SECONDS"
...
def _ensure_api_key(dry_run: bool) -> None:
    if _load_codex_api_key(dry_run=dry_run):
        print(
            f"{CODEX_AUTH_API_KEY_FIELD} found in {_codex_auth_path()}.",
            file=sys.stderr,
        )
        return
    if dry_run:
        _warn(f"{CODEX_AUTH_API_KEY_FIELD} is not set in Codex auth.json; dry-run only.")
        return
    _die(f"{CODEX_AUTH_API_KEY_FIELD} is not set in Codex auth.json.")
...
def _toml_load(path: Path) -> Dict[str, Any]:
    ...
    if parser is None:
        try:
            ...
            _die(f"Reading {path} requires Python 3.11+ or the tomli package.")
    with path.open("rb") as handle:
        return parser.load(handle)  # type: ignore[union-attr]


def _codex_config_path() -> Path:
    explicit = os.getenv(CODEX_CONFIG_ENV)
    if explicit:
...
def _codex_auth_path() -> Path:
    explicit = os.getenv(CODEX_AUTH_ENV)
    if explicit:
        return Path(explicit).expanduser()
...
def _load_codex_api_key(*, dry_run: bool = False) -> str:
    auth_path = _codex_auth_path()
    if not auth_path.exists():
        if dry_run:
            _warn(f"Codex auth file not found: {auth_path}")
            return ""
        _die(
            f"Codex auth file not found: {auth_path}. "
            f"Set {CODEX_AUTH_ENV} to the auth.json path if needed."
        )
    ...
    try:
        ...
        if dry_run:
            _warn(f"Could not parse Codex auth file {auth_path}: {exc}")
            return ""
        _die(f"Could not parse Codex auth file {auth_path}: {exc}")

    if not isinstance(data, dict):
        if dry_run:
            _warn(f"Codex auth file is not a JSON object: {auth_path}")
            return ""
        _die(f"Codex auth file is not a JSON object: {auth_path}")

    api_key = data.get(CODEX_AUTH_API_KEY_FIELD)
    if isinstance(api_key, str) and api_key.strip():
        return api_key.strip()

    if dry_run:
        _warn(f"{CODEX_AUTH_API_KEY_FIELD} not found in {auth_path}")
        return ""
    _die(f"{CODEX_AUTH_API_KEY_FIELD} not found in {auth_path}.")


def _load_model_provider(provider_name: str = DEFAULT_PROVIDER_NAME) -> Dict[str, Any]:
    config_path = _codex_config_path()
    if not config_path.exists():
        _die(
            f"Codex config not found: {config_path}. "
            f"Set {CODEX_CONFIG_ENV} to the config.toml path if needed."
        )

    data = _toml_load(config_path)
    providers = data.get("model_providers")
    if not isinstance(providers, dict):
        _die(f"No [model_providers] table found in {config_path}.")

    provider = providers.get(provider_name)
    if not isinstance(provider, dict):
        _die(f"No [model_providers.{provider_name}] table found in {config_path}.")

    base_url = str(provider.get("base_url") or "").strip()
    if not base_url:
        _die(f"[model_providers.{provider_name}] in {config_path} has no base_url.")

    return {"name": provider_name, "base_url": base_url, "config_path": config_path}


def _request_timeout_seconds() -> float:
    raw = os.getenv(REQUEST_TIMEOUT_ENV)
    if raw is None or not raw.strip():
        return DEFAULT_REQUEST_TIMEOUT_SECONDS
    try:
        timeout = float(raw)
    except ValueError:
        _die(f"{REQUEST_TIMEOUT_ENV} must be a number of seconds.")
    if timeout <= 0:
        _die(f"{REQUEST_TIMEOUT_ENV} must be greater than 0.")
    return timeout
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "AIG/Joinin CLI for explicit image generation or editing via GPT Image models"
        )
    )
