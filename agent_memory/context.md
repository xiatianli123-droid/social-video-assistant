...
- 当前有效背景：短视频分镜工作流分为分镜、HTML、首帧图三个职责；分镜和首帧图任务已结束，当前项目进入 Git 打包与交接阶段。项目内 active skills 为 `short-video-storyboard`、`storyboard-html-delivery`、`storyboard-first-frame-images`、`aig-image-gen`；`.codex/skills/seedance-video/` 已于 2026-06-24 删除，后续不再作为项目依赖。
...
- Brief 面板尺寸：`outputs/storyboard-brief-selector.html` 的选项区最后包含 `画面尺寸` 单选组，默认 `9:16 竖屏`，可选 `1:1 方形`、`4:5 竖版信息流`、`16:9 横屏`，并支持自定义尺寸；生成 brief 会自动包含该字段。
